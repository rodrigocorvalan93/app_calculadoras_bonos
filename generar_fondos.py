...
#   1. Slides de fondos embebibles en el HTML del comite
#   2. Una presentacion PPTX separada
#
# USO:
...
#       template_path   = r"C:\...\Presentacion_Comite_Claude.pptx",  # opcional
#   )
# =============================================================================
...
#   col_metr:  columna (0-indexed) donde estan los valores de metricas (TIR, Duration, Patrimonio)
#   col_pct:   columna (0-indexed) del % de participacion de cada instrumento
#   col_monto: columna (0-indexed) del monto nominal
#   nombre:    nombre para mostrar
#   tipo_vcp:  titulo del grafico de variaciones VCP
...
# Mapeo subcategoria RF_Detalle -> tipo de ajuste para grafico
AJUSTE_MAP = {
    "ARS Fija":               "Tasa Fija",
    ...
    "USD en Dolares":         "Hard-Dollar",
    "USB en Dolares":         "Hard-Dollar",
    "USD-Linked Fija":        "Dolar-Linked",
    "Acciones":               "Acciones",
    "RV":                     "Acciones",
    "Caja":                   "Caja",
    "Money Market":           "Caja",
...
# Categorias nivel 1 que van directo a Caja
CAJA_L1 = {"Caja y Equivalentes", "Caja", "Money Market", "Cuenta Corriente"}

# Colores por tipo de ajuste
AJUSTE_COLORS = {
    "Tasa Fija":    "#0841A5",   # Delta navy — azul fuerte
    "CER/UVA":      "#5B9BD5",   # celeste claro
    "TAMAR":        "#243F6E",   # azul oscuro
    "Duales":       "#8FAADC",   # lila azulado
    "Hard-Dollar":  "#C45911",   # naranja oscuro
    "Dolar-Linked": "#ED7D31",   # naranja claro
    "Acciones":     "#375623",   # verde oscuro
    "Caja":         "#A5A5A5",   # gris neutro
    "Soberanos CER":    "#5B9BD5",
    "Soberanos Tasa Fija": "#0841A5",
    "Soberanos TAMAR":  "#243F6E",
    "Soberanos USD":    "#C45911",
    "ON CER/UVA":       "#8FAADC",
    "ON USD":           "#ED7D31",
    "Subsoberanos CER": "#70AD47",
    "Provinciales TAMAR":"#A9D18E",
    "FF TAMAR":         "#44546A",
}

AJUSTE_ORDER = ["Tasa Fija", "CER/UVA", "TAMAR", "Duales",
                "Hard-Dollar", "Dolar-Linked", "Acciones", "Caja"]
...
def _read_sheet(path: str, sheet: str) -> pd.DataFrame:
    """Lee un sheet de Excel. Si el archivo esta bloqueado (abierto en Excel),
    hace una copia temporal antes de leer."""
    import tempfile, shutil, os
    try:
        return pd.read_excel(path, sheet_name=sheet, header=None, engine="openpyxl")
    except PermissionError:
        # Archivo bloqueado por Excel - copiar a temp y leer desde ahi
...
def _get_metrics(df: pd.DataFrame, col_metr: int) -> dict:
    """Extrae metricas clave del encabezado del sheet."""
    def _v(row):
...
def _get_composition(df: pd.DataFrame, col_pct: int) -> dict:
    """
    Recorre el arbol de tenencias (nivel 1/2/4) y acumula % por tipo de ajuste.
    ...
    for i in range(27, min(len(df), 300)):
        ...
        if nivel == 1.0:
            current_l2 = None
            # ¿Es una categoria de Caja directo?
        ...
        # nivel 4 - instrumento individual: no sumamos (ya esta en nivel 2)

    # Limpiar valores muy pequenos
...
def _get_top5(df: pd.DataFrame, col_pct: int) -> list:
    """
    Extrae los top 5 instrumentos por % de participacion.
    ...
    for i in range(27, min(len(df), 300)):
        ...
        # Fecha de vencimiento desde col 3 (Av Life en anos -> approx date)
...
def _fmt_tir(v: float, is_cer: bool = False) -> str:
    if v is None: return "-"
    if is_cer:
        real = v - 0.20  # approx: TIR real ≈ TIR nominal - inflacion esperada
...
_FONDO_CSS = """
/* Slides fondos - layout referencia Delta */
.fd-slide-inner {
  flex: 1; min-height: 0;
  padding: 16px 20px 8px;
  font-family: Calibri,'Segoe UI',Arial,sans-serif;
  display: flex; flex-direction: column; gap: 10px;
  overflow: hidden; box-sizing: border-box; background: #e8ecf0;
}
.fd-header { flex-shrink: 0; }
.fd-title    { font-size: clamp(28px,3.8vh,46px); font-weight: 700; color: #0841A5; line-height: 1.1; }
.fd-subtitle { font-size: clamp(14px,1.8vh,20px); color: #0841A5; font-weight: 400; margin-top: 2px; }

/* Body: KPI izq + grilla 2x2 */
.fd-body {
  flex: 1; min-height: 0;
  display: grid;
  grid-template-columns: clamp(155px,17%,205px) 1fr;
  gap: 12px;
}
.fd-kpi {
  background: #6b7c8e; border-radius: 4px;
  padding: clamp(14px,2.2vh,28px) 16px;
  display: flex; flex-direction: column;
  justify-content: center; align-items: center;
  gap: clamp(14px,2.2vh,28px); color: white; text-align: center;
}
.fd-kpi-item  { display: flex; flex-direction: column; align-items: center; }
.fd-kpi-label { font-size: clamp(11px,1.2vh,14px); font-weight: 700; text-transform: uppercase; letter-spacing: .9px; color: rgba(255,255,255,.65); margin-bottom: 3px; }
.fd-kpi-value { font-size: clamp(20px,2.8vh,36px); font-weight: 700; color: white; line-height: 1.05; }
.fd-kpi-date  { font-size: clamp(11px,1.2vh,14px); color: rgba(255,255,255,.5); }
...
/* Cuadrante con label vertical izquierdo */
.fd-quadrant {
...
.fd-q-label {
  background: #0841A5; color: white;
  writing-mode: vertical-rl; text-orientation: mixed; transform: rotate(180deg);
  font-size: clamp(10px,1.1vh,13px); font-weight: 700;
  text-transform: uppercase; letter-spacing: .9px;
  padding: 14px 7px; flex-shrink: 0; min-width: 30px;
...
/* Tenencias */
.fd-top5 { width: 100%; border-collapse: collapse; font-family: Calibri,'Segoe UI',Arial; font-size: clamp(13px,1.6vh,18px); }
.fd-top5 tr:nth-child(odd)  { background: #f4f6f9; }
.fd-top5 tr:nth-child(even) { background: white; }
.fd-top5 td { padding: clamp(6px,1vh,12px) 10px; vertical-align: middle; }
.fd-top5 .tk   { font-weight: 600; color: #1a2535; }
.fd-top5 .pct  { background: #6b7a8a; color: white; border-radius: 3px; padding: 3px 14px; font-weight: 700; display: block; text-align: center; font-size: clamp(13px,1.6vh,18px); white-space: nowrap; }
.fd-top5 .fecha { color: #7a8a9a; font-size: clamp(12px,1.4vh,16px); }
.vcp-inputs-slide-inner { flex: 1; overflow-y: auto; padding: 20px 36px 16px; font-family: 'Calibri','Segoe UI',Arial; }
.vcp-inputs-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 18px; margin-top: 14px; }
.vcp-inp-fondo { background: white; border: 1px solid #d0d9e4; border-radius: 6px; overflow: hidden; }
.vcp-inp-fondo-title { background: #0f2557; color: white; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: .5px; padding: 6px 12px; }
.vcp-inp-sub { font-weight: 400; opacity: .7; font-size: 10px; text-transform: none; letter-spacing: 0; }
.vcp-inp-tbl { width: 100%; border-collapse: collapse; font-size: 12px; font-family: 'Calibri','Segoe UI',Arial; }
.vcp-inp-tbl thead th { background: #f0f4f8; color: #5f7080; font-size: 10px; font-weight: 700; text-transform: uppercase; padding: 5px 10px; text-align: center; border-bottom: 1px solid #d0d9e4; }
.vcp-inp-tbl thead th:first-child { text-align: left; }
.vcp-inp-tbl tbody tr { border-bottom: 1px solid #edf0f3; }
.vcp-inp-tbl tbody tr:last-child { border-bottom: none; }
.vcp-inp-lbl { padding: 5px 10px; color: #333; font-weight: 600; }
.vcp-inp-tbl td input { width: 90%; border: 1px solid #d0d9e4; border-radius: 3px; padding: 3px 6px; font-size: 12px; text-align: center; font-family: 'Calibri','Segoe UI',Arial; box-sizing: border-box; display: block; margin: 4px auto; }
.vcp-inp-tbl td input:focus { outline: none; border-color: #1e6fba; }
.vcp-inputs-note { font-size: 11px; color: #7a90a4; margin-top: 5px; font-style: italic; }
/* Asegurar flex-direction:column en slide-chapter.active (el template no lo pone en .active) */
.slide-chapter.active { flex-direction: column !important; }
.fd-slide.active { flex-direction: column !important; }

/* Claude iframe usa @media print - override para restaurar layout de pantalla */
@media print {
  ...
  .slides-area { display: flex !important; flex-direction: column !important; flex: 1 !important; overflow: hidden !important; }
  .slide-chapter { display: none !important; height: 100% !important; flex-direction: column !important; overflow: hidden !important; }
  .slide-chapter.active { display: flex !important; }
  .slide-cover { display: none !important; }
  .slide-cover.active { display: flex !important; }
  .fd-slide { display: none !important; }
...
(function(){{
  function drawPie_{canvas_id}(){{
    ...
    if (!W || W < 10 || !H || H < 10) {{ setTimeout(drawPie_{canvas_id}, 150); return; }}
    ...
    for (var i = 0; i < data.length; i++) {{
      ...
      if (data[i] / total > 0.04) {{
        ...
        ctx.font = 'bold 12px Barlow,Calibri,Arial';
    ...
    ctx.font = '12px Barlow,Calibri,Arial';
...
(function(){{
  function drawBar_{canvas_id}(){{
    ...
    var W = cv.parentElement.clientWidth;
    var H = cv.parentElement.clientHeight;
    if (!W || W < 10 || !H || H < 10) {{ setTimeout(drawBar_{canvas_id}, 150); return; }}
    var dpr = window.devicePixelRatio || 1;
    cv.width = W * dpr; cv.height = H * dpr;
    cv.style.width = W + 'px'; cv.style.height = H + 'px';
    var ctx = cv.getContext('2d');
...
def _vcp_chart_js(fondo_id: str) -> str:
    """JS para el grafico de variaciones VCP - lee inputs del formulario."""
    return f"""
<script>
(function(){{
  function drawVcp_{fondo_id}(){{
    ...
    for (var i = 0; i < n; i++) {{
      ...
      // Label entidad
      ctx.fillStyle = '#444'; ctx.font = '11px Barlow,Calibri,Arial'; ctx.textAlign='center';
      ctx.fillText(labels[i], gx + barW + 2 + barW/2, H - 18);
    }}

    // Leyenda — abajo a la izquierda, sobre el eje
    var legY = H - 6;
    ctx.fillStyle = colors1;  ctx.fillRect(ML, legY - 9, 10, 8);
    ctx.fillStyle = '#333'; ctx.font = '10px Barlow,Calibri,Arial'; ctx.textAlign='left';
    ctx.fillText('Ult. semana', ML+13, legY);
    ctx.fillStyle = colors30; ctx.fillRect(ML+88, legY - 9, 8, 7);
    ctx.fillText('Ult. 30d', ML+100, legY);
  }}
...
def _build_fondo_slide(fondo_data: dict, slide_num: int, total_slides: int) -> str:
    ...
    pie_js = _pie_chart_js(f"pie_{fid}", pie_labels, pie_values, pie_colors)
    bar_js = _bar_chart_js(f"bar_{fid}", pie_labels, pie_values, "#1e6fba")
    ...
    return (
        ...
        # KPI
        f'      <div class="fd-kpi">'
        f'<div class="fd-kpi-item"><div class="fd-kpi-label">Patrimonio</div><div class="fd-kpi-value">{pn_str}</div></div>'
        f'{kpi_tir}'
        f'<div class="fd-kpi-item"><div class="fd-kpi-label">Duration*</div><div class="fd-kpi-value">{dur_str}</div></div>'
        f'<div class="fd-kpi-item"><div class="fd-kpi-label">Datos al</div><div class="fd-kpi-date">{fecha_str}</div></div>'
        f'</div>\n'
        # Grid 2x2 con labels verticales
        f'      <div class="fd-right">\n'
        # Q1 Composicion
        f'        <div class="fd-quadrant">'
        f'<div class="fd-q-label">Composicion General</div>'
        f'<div class="fd-q-body" style="padding:2px">'
        f'<canvas id="pie_{fid}" style="width:100%;height:100%;display:block"></canvas>'
        f'</div></div>\n'
        # Q2 Tenencias
        f'        <div class="fd-quadrant">'
        f'<div class="fd-q-label">Principales Tenencias</div>'
        f'<div class="fd-q-body" style="padding:6px 10px;align-items:center;flex-direction:column;justify-content:center">'
        f'<table class="fd-top5"><tbody>{top5_rows}</tbody></table>'
        f'</div></div>\n'
        # Q3 Tipo de ajuste
        f'        <div class="fd-quadrant">'
        f'<div class="fd-q-label">Tipo de Ajuste</div>'
        f'<div class="fd-q-body" style="padding:4px">'
        f'<canvas id="bar_{fid}" style="width:100%;height:100%;display:block"></canvas>'
        f'</div></div>\n'
        # Q4 VCP
        f'        <div class="fd-quadrant">'
        f'<div class="fd-q-label">Variaciones VCP</div>'
        f'<div class="fd-q-body" style="padding:4px;display:block">'
        f'<input type="hidden" id="vcp-lbl-{fid}" value="{peers_lbl}">'
...
def generar_fondos_nav_items(fondos: list = None) -> str:
    ...
    Se insertan despues del nav-item 07 (Snapshot).
    ...
    for i, fd in enumerate(fondos, 1):
      ...
      <span class="nav-label">{fd['nombre']}<br>
        <span style="font-size:10px;opacity:.6">Composicion . Top 5</span>
      </span>
    </div>""")
...
def generar_fondos_html(
    ...
    # Script de navegacion goToId() - compatibiliza con goTo() del reporte
    goto_js = """
<script>
// goToId - wrapper de la navegacion unificada
// La funcion goToId real esta definida en el HTML por _unify_navigation()
...
if (typeof goToId === 'undefined') {
  function goToId(targetId) {
    ...
    if (target) {
      ...
      // Forzar reflow para que height:100% se compute correctamente
      void target.offsetHeight;
      // Fix definitivo: forzar flex-direction:column + altura explicita en px
      (function() {
        var inp = document.getElementById('fd-slide-inputs');
        if (!inp || !inp.classList.contains('active')) return;
        inp.style.flexDirection = 'column';
        var inpInner = document.getElementById('inp-inner-scroll');
        if (!inpInner) return;
        var inpH = inp.getBoundingClientRect().height || window.innerHeight;
        var pbH = (inp.querySelector('.progress-bar') || {}).offsetHeight || 3;
        var ftH = (inp.querySelector('.slide-footer') || {}).offsetHeight || 36;
        var h = Math.max(200, inpH - pbH - ftH - 4);
        inpInner.style.height = h + 'px';
        inpInner.style.overflowY = 'auto';
        inpInner.style.display = 'block';
      })();
      setTimeout(function() {
        ...
        if (active) {
          ...
          if (typeof window['drawPie_pie-'+fid]==='function') window['drawPie_pie-'+fid]();
          if (typeof window['drawBar_bar-'+fid]==='function') window['drawBar_bar-'+fid]();
...
def generar_inputs_slide(fondos: list = None) -> str:
    """
    Genera un slide unico (cap 12) con todos los inputs VCP de todos los fondos.
    Se inserta al final del HTML, despues de los fd-slides de fondos.
    """
    fondos = fondos or FONDOS
    n_fondos = len(fondos)

    # Para cada fondo: fila de inputs
    rows_html = ""
    for fd in fondos:
        fid = fd["id"]
        entities = [
            ("fondo",     fd["peers_label"]),
            ("benchmark", "Benchmark"),
            ("peers",     "Peers"),
            ("industria", "Industria"),
        ]
        rows_html += f"""
<div class="vcp-inp-fondo">
  <div class="vcp-inp-fondo-title">{fd['nombre']} <span class="vcp-inp-sub">{fd['tipo_vcp']}</span></div>
  <table class="vcp-inp-tbl">
    <thead><tr>
      <th></th><th>Ult. semana (%)</th><th>Ult. 30d (%)</th>
    </tr></thead>
    <tbody>"""
        for eid, lbl in entities:
            rows_html += f"""
      <tr>
        <td class="vcp-inp-lbl">{lbl}</td>
        <td><input type="number" step="0.01" placeholder="0.0"
            id="vcp-{fid}-s1-{eid}"
            oninput="if(typeof drawVcp_{fid}==='function') drawVcp_{fid}()"></td>
        <td><input type="number" step="0.01" placeholder="0.0"
            id="vcp-{fid}-s30-{eid}"
            oninput="if(typeof drawVcp_{fid}==='function') drawVcp_{fid}()"></td>
      </tr>"""
    ...
    css = """
<style>
/* Override @media print rules that break the inputs slide in Claude iframe */
@media print {
  #fd-slide-inputs { display: block !important; height: auto !important; }
  #fd-slide-inputs > div:not(.progress-bar):not(.slide-footer) { 
    display: block !important; overflow: visible !important; 
    max-height: none !important;
  }
}
/* Screen: normal flex layout */
@media screen {
  #fd-slide-inputs {
    display: none;
  ...
  #fd-slide-inputs.active {
    display: flex !important;
  }
  #fd-slide-inputs > .inp-body {
    flex: 1;
    min-height: 0;
    overflow-y: auto;
  }
}
.vcp-inputs-slide-inner {
  flex: 1; overflow-y: auto; padding: 24px 40px 20px;
  font-family: 'Calibri','Segoe UI',Arial,sans-serif;
}
.vcp-inputs-grid {
  display: grid;
...
.vcp-inp-sub {
  font-weight: 400; opacity: .7; font-size: 10px;
  text-transform: none; letter-spacing: 0;
}
.vcp-inp-tbl {
  width: 100%; border-collapse: collapse;
  font-size: 12px; font-family: 'Calibri','Segoe UI',Arial,sans-serif;
}
.vcp-inp-tbl thead th {
  background: #f0f4f8; color: #5f7080;
...
.vcp-inp-tbl tbody tr:last-child { border-bottom: none; }
.vcp-inp-lbl { padding: 5px 10px; color: #333; font-weight: 600; }
.vcp-inp-tbl td input {
  width: 90%; border: 1px solid #d0d9e4; border-radius: 3px;
  padding: 3px 6px; font-size: 12px; text-align: center;
  font-family: 'Calibri','Segoe UI',Arial,sans-serif;
  box-sizing: border-box; display: block; margin: 4px auto;
}
.vcp-inp-tbl td input:focus { outline: none; border-color: #1e6fba; }
.vcp-inputs-note {
  font-size: 11px; color: #7a90a4; margin-top: 6px;
  font-style: italic;
}
</style>"""
...
<div class="slide-chapter fd-slide" id="fd-slide-inputs">
  ...
  <div id="inp-inner-scroll" style="overflow-y:auto;padding:16px 28px 12px;font-family:'Calibri','Segoe UI',Arial,sans-serif;">
    <div style="font-family:'Calibri','Segoe UI',Arial;font-size:22px;font-weight:700;color:#0f2557;line-height:1.1">
      Inputs - Variaciones VCP
    </div>
    <div style="font-size:12px;color:#5f7080;margin-top:3px">
      Completa los valores para todos los fondos. Los graficos de cada slide se actualizan automaticamente.
    </div>
    <div class="vcp-inputs-note">Valores en % directo (ej: 0.9 = 0.9%)</div>
    <div class="vcp-inputs-grid" style="min-height:200px">
      {rows_html}
    </div>
  </div>
  <div class="slide-footer">
    <div class="slide-footer-left">Delta Asset Management . Inputs VCP . Uso interno</div>
    <div class="slide-nav-btns">
      <button class="slide-nav-btn" onclick="goToId('slide-7')"><- Volver al reporte</button>
    </div>
...
def generar_fondos_pptx(
    ...
    """
    Genera una presentacion PPTX con las slides de fondos.
    Si se provee template_path, la usa como base (mantiene el diseno).
    """
    try:
        ...
        print("[generar_fondos] python-pptx no instalado. Instala con: pip install python-pptx")
        return None
    ...
    # Usar layout en blanco (ultimo disponible) o el primero
