...
async def historicos_semanal_csv(dias: int = 7):
    ...
    (';' separador, coma decimal — abre directo en Excel AR). El builder
    vive en services.quepaso_report (compartido con el mail de cierre);
    sale del cache de weekly_segments → costo ~0."""
    ...
    loop = asyncio.get_running_loop()
    csv = await loop.run_in_executor(None, quepaso_report.csv_es_ar, dias)
