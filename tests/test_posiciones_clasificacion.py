...
def test_base_delta_especies_ajuste_tasa_y_tipo() -> None:
    ...
    assert C.clasificar("ON FANTASMA TAMAR", "Renta Fija", on_tamar, "ZONT1") == ("ARS TAMAR", "base")
    assert C.clasificar("ON X", "Renta Fija", {"Ajuste": "En pesos", "Tasa": "Fija"}) == ("ARS Fija", "base")
...
def test_memo_no_confunde_distintas_filas_de_la_base() -> None:
    ...
    assert (a, b, c) == (("ARS TAMAR", "base"), ("ARS Fija", "base"), ("Renta Fija", "sin_regla"))
...
def test_composicion_estilo_delta_ahorro(monkeypatch) -> None:
    ...
    class _B:                                   # ficha mínima
        def __init__(self, tipo="VARIABLE", index="TAMAR"):
            self.industria = ""
            ...
            self.clasificacion = "Corporativo TAMAR"

    fichas = {"ONT1": _B(), "ONF1": _B("FIJA", "")}
    ...
    assert cats == {
        "ARS TAMAR": 127.0, "Plazos Fijos": 44.0, "Fideicomisos TAMAR/BADLAR": 43.0, "Caución": 24.0,
        "Otros Activos Netos": 19.0, "ARS Fija": 16.0, "FCI Money Markets": 10.0,
    ...
    # un fondo soberano con fichas no cambia en nada
    s13 = P._composicion_summary([{"cod_fondo": 13, "cod_delta": "ONT1", "especie": "X", "valor": 5.0,
                                   "clase": "Títulos Públicos"}], pn=10.0)
    assert [(r["cat"], r["monto"]) for r in s13["Categoría"]] == [("ARS TAMAR", 5.0)]
