...
class Bono:
    def __init__(self, data):
        ...
        self.legislacion = data.get("Legislación")
        ...
        self.fechas_devengo_intereses_habil = fechas_devengo_intereses_habil
        ...
        if self.step_up == True:
            ...
            elif self.tipo_tasa_interes == 'VARIABLE' and self.index == "BADLAR":
            ...
            elif self.tipo_tasa_interes == 'VARIABLE' and self.index == "TAMAR":
                # Calcular el promedio de la serie badlar para cada intervalo de fechas de pago (fechas_devengo_intereses_habil tiene 29 incluye emision para promediar)
                index_premium = []
                for i in range(len(fechas_devengo_intereses_habil) - 1):
                    inicio = fechas_devengo_intereses_habil[i]
                    fin = fechas_devengo_intereses_habil[i + 1]
                    promedio = inputs['tamar_proyectado'][(inputs['tamar_proyectado'].index >= inicio) & (inputs['tamar_proyectado'].index < fin)]['TAMAR'].mean() # se puede mejorar a una version donde la proyeccion sea segun el settlement
                    index_premium.append(promedio)
            elif self.tipo_tasa_interes == "VARIABLE_CAP" and self.index == "TAMAR":
                # Calcula capitalizacion de interes para tamar
                index_premium = []
                for i in range(len(fechas_devengo_intereses_habil) - 1):
                    inicio = fechas_devengo_intereses_habil[i]
                    fin = fechas_devengo_intereses_habil[i + 1]
                    ...
                    self.tamar_tem = index_premium #para chequear quitar quitar
            ...
            if self.tipo_tasa_interes == "VARIABLE_CAP" and self.index == "TAMAR":
                cupon_aplicable = ((1+tamar_tem_100/100)**(dias_entre_cupones_anual*12))-1
                intereses = cupon_aplicable * valores_residuales 
                self.intereses = intereses
            else:
                cupon_aplicable = [tasavar + self.cupon_spread for tasavar in index_premium] #este es el importante
                intereses = dias_entre_cupones_anual * (cupon_aplicable) / 100 * valores_residuales # cuenta vectorial
                self.intereses = intereses
    ...
    def generate_cashflows(self, settlement_date=None):
        ...
        if self.ajuste_sobre_capital== None:
            ...
            for i in range(self.cupones):
                ...
                ratio_aplicable = ajuste_actual / ajuste_base
                ajuste_aplicable.append(ratio_aplicable)
                indice_aplicable.append((fecha_ajuste_actual,ajuste_actual))

            self.indice_aplicable = indice_aplicable
            self.fecha_cer_base = fecha_cer_base
            self.ajuste_aplicable = ajuste_aplicable

        elif self.ajuste_sobre_capital == "UVA PROYECTADO":
            ajuste_aplicable = []
            indice_aplicable = []
            # Fechas de ratio offseteado
            fecha_uva_base = n_dias_laborales(pd.to_datetime(self.emision),self.dias_lag_ajuste_base)
            ajuste_base = inputs['uva_proyectado'].loc[fecha_uva_base, 'UVA']
            self.ajuste_base = ajuste_base
            fecha_ajuste_si_no_hay_dato = n_dias_laborales(settlement_date, self.dias_lag_ajuste)
            for i in range(self.cupones):
                fecha_ajuste_actual = n_dias_laborales(self.fechas_cupon[i],self.dias_lag_ajuste)
                ajuste_actual = inputs['uva_proyectado'].loc[fecha_ajuste_actual, 'UVA'] if fecha_ajuste_actual in inputs['uva_proyectado'].index else inputs['uva_proyectado'].loc[fecha_ajuste_si_no_hay_dato]['UVA']
                ratio_aplicable = ajuste_actual #/ ajuste_base
                ajuste_aplicable.append(ratio_aplicable)
                indice_aplicable.append((fecha_ajuste_actual,ajuste_actual))

            self.indice_aplicable = indice_aplicable
            self.fecha_uva_base = fecha_uva_base
            self.ajuste_aplicable = ajuste_aplicable

        elif self.ajuste_sobre_capital == "UVA":
            ajuste_aplicable = []
            # Fechas de ratio offseteado
            fecha_uva_base = n_dias_laborales(pd.to_datetime(self.emision),self.dias_lag_ajuste_base)
            ajuste_base = inputs['UVA'].loc[fecha_uva_base, 'UVA']
            self.ajuste_base = ajuste_base
            fecha_ajuste_si_no_hay_dato = n_dias_laborales(settlement_date, self.dias_lag_ajuste)
            for i in range(self.cupones):
                fecha_ajuste_actual = n_dias_laborales(self.fechas_cupon[i],self.dias_lag_ajuste)
                ajuste_actual = inputs['UVA'].loc[fecha_ajuste_actual, 'UVA'] if fecha_ajuste_actual in inputs['UVA'].index else inputs['UVA'].loc[fecha_ajuste_si_no_hay_dato]['UVA']
                ratio_aplicable = ajuste_actual #/ ajuste_base
                ajuste_aplicable.append(ratio_aplicable)
    ...
    def genera_ticket(self, precio_ticket, nominales_ticket=1000000, settlement_date=None, tipo_de_cambio_aplicable=None):
        """
        Genera un ticket con el precio de mercado.

        Parámetros:
        - precio_ticket: Precio de mercado según convención.
        - nominales_ticket: Nominales del ticket (por defecto 1,000,000).
        - settlement_date: Fecha de liquidación (opcional).
        - tipo_de_cambio_aplicable (exclusivo para DLK)
        ...
        # Calcular interés corrido y TIREA
    ...
    def genera_ticket_tna(self, tna_ticket, nominales_ticket=1000000, settlement_date=None, tipo_de_cambio_aplicable=None):
        """
        ...
        tir = tna_a_tir(tna_ticket, int(cnv_dias_al_vto), int(self.convencion_base))
    ...
    def genera_ticket_tna_margen(self, tna_margen_ticket, nominales_ticket=1000000, settlement_date=None):
        """
        Genera un ticket en base a la Tasa Nominal Anual (TNA) según la convención del bono. (SOLO FUNCIONA CON BADLAR)

        Parámetros:
        - tna_ticket: TNA MARGEN BADLAR según convención.
        - nominales_ticket: Nominales del ticket (por defecto 1,000,000).
        - settlement_date: Fecha de liquidación (opcional).
        ...
        # Calcular interés corrido y precio
        self.calcula_intereses_corridos(settlement_date)
        ajuste_aplicable = None
        if self.tipo_tasa_interes == "VARIABLE" and self.index == "BADLAR":
           ajuste_aplicable = inputs['badlar'].tail(5)["BADLAR"].mean()
        if self.tipo_tasa_interes == "VARIABLE" and self.index == "TAMAR":
           ajuste_aplicable = inputs['tamar'].tail(5)["TAMAR"].mean()
        

        tna_ticket = ajuste_aplicable / 100 + tna_margen_ticket
        cnv_dias_al_vto = (self.vencimiento - self.fecha_settlement).days if self.cnv_tna == 'plazo remanente' else self.cnv_tna
        ...
        if self.tipo_tasa_interes == "VARIABLE":
            ticket.update({
                'Tasa Aplicable': f'{self.index} {ajuste_aplicable}%',
                'Margen TNA': self.tna - ajuste_aplicable / 100
            })
    ...
    def genera_ticket_blotter(self, fondo, operacion, contraparte, precio_ticket, nominales_ticket=1000000, settlement_date=None):
        ...
        ticket = {
            ...
            'Principal': "{:,.2f}".format(nominales_ticket * self.precio_clean * self.valor_residual/self.valor_nominal),
            'Interés': nominales_ticket * self.intereses_corridos,
            'Días Devengados': self.dias_corridos,
        }
    ...
    def comparar_precio(self, bono2, precio_ticket_1, precio_ticket_2, nominales_ticket=1000000, settlement_date=None):
        """
        Método para comparar rendimientos entre bonos

        Parámetros:
        - bono2: ticker bono 2
        - precio_ticket_1: El precio según convención del bono 1
        - precio_ticket_2: El precio según convención del bono 2
        - nominales_ticket: Por defecto en 1mm
        - settlement_date (opcional): La fecha de liquidación para el cálculo de los flujos de caja.
          Si no se proporciona, se calcula automáticamente t+convención.

        Retorna:
        - tabla comparando bonos
        """
        # Calcula interés corrido y tir
        ticket1 = self.genera_ticket(precio_ticket_1, nominales_ticket, settlement_date)

        bono2.calcula_tirea(precio_ticket_2)
        px_dirty2 = bono2.precio # para calcular los VN de la otra pata
        ticket2 = bono2.genera_ticket(precio_ticket_2, nominales_ticket*self.precio/px_dirty2, settlement_date)

        return pd.concat((ticket1, ticket2),axis=1)

    def comparar_tna(self, bono2, tna_ticket_1, tna_ticket_2, nominales_ticket=1000000, settlement_date=None):
        """
        Método para comparar rendimientos entre bonos

        Parámetros:
        - bono2: ticker bono 2
        - tna_ticket_1: El tna según convención del bono 1
        - tna_ticket_2: El tna según convención del bono 2
        - nominales_ticket: Por defecto en 1mm
        - settlement_date (opcional): La fecha de liquidación para el cálculo de los flujos de caja.
          Si no se proporciona, se calcula automáticamente t+convención.
        ...
        if bono2.cnv_tna == 'plazo remanente': # verifica si A no es regular con frecuencias 2,4,6,12,48,365
            cnv_dias_al_vto_2 = (bono2.vencimiento - settlement_date_dt).days
        else:
            cnv_dias_al_vto_2 = self.cnv_tna


        # Calcula interés corrido y tir
        ticket1 = self.genera_ticket_tna(tna_ticket_1, nominales_ticket, settlement_date)
