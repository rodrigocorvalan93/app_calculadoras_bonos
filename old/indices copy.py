...
# Extrae todos los índices necesarios para cálculos de renta fija argenta.
...
def _calcular_start_date_por_backup(backup: dict, default_days_back: int = 365):
    ...
    - Si hay backup:
        * si la MÍNIMA última fecha de las series < hoy → hay al menos una serie atrasada,
          pedimos desde (esa mínima + 1) hasta hoy.
        * si la MÍNIMA última fecha >= hoy → todas las series están al día, no pedimos nada nuevo.
    """
    ...
    for k in ("a3500", "badlar", "CER", "UVA", "tamar", "inflamom"):
        ...
        if df is not None and isinstance(df, pd.DataFrame) and not df.empty:
            ult = pd.to_datetime(df.index.max()).date()
    ...
    ultima_min = min(candidatos)

    if ultima_min >= hoy:
        # ya actualizado: todas las series tienen último dato >= hoy
        return hoy.strftime("%Y-%m-%d"), hoy.strftime("%Y-%m-%d"), True

    # hay al menos una serie atrasada
...
def main():
    ...
    if not combined_df_cer.empty:
        ...
        print(f"El último CER: {combined_df_cer.iloc[-1]}")
    else:
    ...
    if not combined_df_inflamom.empty:
        df_inflamom_combinado = proy_inf_df.combine_first(combined_df_inflamom)
    else:
    ...
    # 8) Guardar backup (solo histórico + inflación)
    backup_payload = {
        "a3500": combined_df_a3500,
        "badlar": combined_df_badlar,
        "tamar": combined_df_tamar,
