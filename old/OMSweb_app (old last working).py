...
def main():
    ...
    # ─────────────────────────
    with tab_curvas:
        st.caption(f"Actualizado: {datetime.now().strftime('%H:%M:%S')}  |  Plazo: {plazo}")
            ...
            if compact:
                with st.expander(title, expanded=(c.key in ("cer", "lecap"))):
                    if df is None or df.empty:
                        ...
                        st.dataframe(style_curvas(df), use_container_width=True, height=520)
            else:
                st.subheader(title)
                if df is None or df.empty:
                    st.info("Sin datos (mercado cerrado o sin respuesta de marketdata).")
    ...
    with tab_mercado:
        ...
        dfm = load_curve_market_table(username, password, curve_key, plazo)
        if dfm is None or dfm.empty:
            st.info("Sin datos (mercado cerrado o sin respuesta de marketdata).")
        else:
            st.dataframe(style_mercado(dfm), use_container_width=True, height=680)
    ...
    with tab_fwds:
        ...
        with col1:
            ...
            if fwd_cer.empty:
                ...
                st.dataframe(style_forwards(fwd_cer), use_container_width=True, height=520)
        ...
        with col2:
            ...
            if fwd_lec.empty:
                ...
                st.dataframe(style_forwards(fwd_lec), use_container_width=True, height=520)
        ...
        if raw is None or raw.empty:
            ...
            with colA:
                ...
                if df_min.empty:
                    ...
                    st.dataframe(style_futuros(df_min), use_container_width=True, height=560)
            ...
            with colB:
                ...
                if df_may.empty:
                    ...
                    st.dataframe(style_futuros(df_may), use_container_width=True, height=560)
        ...
        if df_last is None or df_last.empty:
            ...
            with colB:
                if mode.startswith("Nivel"):
                    ...
                    pts_editor = st.data_editor(pts_default, num_rows="dynamic", use_container_width=True)
            ...
            if be_df.empty:
                ...
                if go is not None:
                    ...
                    fig_be_tem.add_trace(go.Bar(
                        x=be_df["Código"],
                        y=_yield_pct_points(be_df["BE TEM"]),
                        name="BE TEM",
                        marker_color="#e74c3c",
